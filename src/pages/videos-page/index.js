import React from "react";
import YouTube from "react-youtube";
import NavBar from "../../components/nav-bar";
import Slider from "react-slick";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import { RightOutlined, LeftOutlined } from "@ant-design/icons";
import "./style.css";
import { Link } from "react-router-dom";
export const VideoPlayer = ({ videoId, title }) => {
  const opts = {
    height: "195",
    width: "320",
    playerVars: {
      autoplay: 0, // 1 for autoplay
    },
  };

  return (
    <div
      className="card border m-1 bg-light d-flex flex-column align-items-stretch"
      style={{ padding: "0", height: "300px" }}
    >
      <YouTube videoId={videoId} opts={opts} />

      <p
        style={{ fontWeight: "bold", width: "320px" }}
        className="mt-1 mb-3 px-3 text-left"
      >
        {title}
      </p>
    </div>
  );
};

const VideoCarousel = ({ videos }) => {
  const slider = React.useRef(null);
  const settings = {
    infinite: true,
    slidesToShow: 4,
    slidesToScroll: 4,

    responsive: [
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: 3,
          slidesToScroll: 3,
          infinite: true,
          dots: true,
        },
      },
      {
        breakpoint: 600,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 2,
          initialSlide: 2,
        },
      },
      {
        breakpoint: 480,
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1,
        },
      },
    ],
  };

  return (
    <div>
      <Slider ref={slider} {...settings}>
        {videos.map((video) => (
          <VideoPlayer
            key={video.videoId}
            videoId={video.videoId}
            title={video.title}
          />
        ))}
      </Slider>
      <div
        style={{
          textAlign: "center",
          marginTop: "15px",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          gap: "10px",
        }}
      >
        <LeftOutlined onClick={() => slider?.current?.slickPrev()} />

        <RightOutlined onClick={() => slider?.current?.slickNext()} />
      </div>
    </div>
  );
};
const categoryNames = [
  "GMAT Quant videos",
  "GMAT Verbal videos",
  "GMAT IR",
  "Profile building",
  "Essays for college admissions",
  "College admits and successful students",
  "Others",
];
const VideosPage = () => {
  const categories = [
    [
      {
        videoId: "lhfKDKdtNGU",
        title: "GMAT Data sufficiency 1 - Saahil Chouhan",
      },
      {
        videoId: "WdvpswptBC4",
        title: "GMAT Arithmetic Class 1 - Ankit",
      },
      {
        videoId: "eA9O1NsYZ1A",
        title: "GMAT Data Sufficiency 1 - Ankit",
      },
      {
        videoId: "js2dAijpU3k",
        title: "GMAT Permutation and combination - Parth Joshi",
      },
      {
        videoId: "3qL7B5f0QCg",
        title: "GMAT Arithmetic - Parth Joshi",
      },
      {
        videoId: "Kb9PDQuJJBk",
        title: "GRE Arithmetic - Hetal Salhotra",
      },
      {
        videoId: "JHjFO2ztHrc",
        title: "GRE Quant - Hetal Salhotra",
      },
      {
        videoId: "9CN_Vm9MttA",
        title: "GRE Quant 2 - Hetal Salhotra",
      },
      {
        videoId: "flGfWT9YsCI",
        title: "GRE Quant 3 - Hetal Salhotra",
      },
      {
        videoId: "VNSbqFRCGrI",
        title: "GRE Quant and RC 5 question solving - Hetal Salhotra",
      },
      {
        videoId: "6gqjWTdJosg",
        title: "GRE Quant 5 - Hetal Salhotra",
      },
      {
        videoId: "VRPiLDEwghQ",
        title: "GRE - Quant 6 -  Geometry - Hetal Salhotra",
      },

      {
        videoId: "m45NnbfT5DM",
        title: "GMAT DATA SUFFICIENCY BY GHANSHYAM- PARTH",
      },

      {
        videoId: "s2CTdzlkDg0",
        title: "GMAT Arithmetic - Nithin Reddy",
      },
      {
        videoId: "4gTMlekIq8A",
        title: "GMAT Data Sufficiency - Nithin Reddy",
      },
      {
        videoId: "M-TFPvTEoWw",
        title: "GMAT Permutation and Combination - Anubhav Gupta",
      },
      {
        videoId: "QVFHs6gCPeM",
        title: "GMAT-Arithmetic Anubhav Gupta",
      },
      {
        videoId: "DKwHPUSZjGI",
        title: "19/03/2022 DATA SUFFICIENCY-ASITH",
      },
      {
        videoId: "1ACUmF2po78",
        title: "ASITH GEOMETRY 14 MARCH (KIRAN)",
      },
      {
        videoId: "mTem1xfV084",
        title: "ASITH'S GMAT CLASS GEOMETRY- (KIRAN)",
      },
      {
        videoId: "s7R4u67_4gk",
        title: "GMAT ALGEBRA - ASITH",
      },
      {
        videoId: "JXMS6toswhs",
        title: "GMAT Algebra - Anubhav Gupta",
      },
      {
        videoId: "B5DWEVYMo0c",
        title: "GMAT Algebra - Nithin Reddy",
      },
      {
        videoId: "uVIp3mQEuW4",
        title: "GMAT Geometry - Nithin Reddy",
      },
      {
        videoId: "iRzljyyaueM",
        title: "GMAT Geometry 2 - Nithin Reddy",
      },
      {
        videoId: "hO0zS2ektuY",
        title: "GMAT Probability - Nithin Reddy",
      },
      {
        videoId: "ASqEO1FAyVc",
        title: "GMAT Probability - Parth",
      },
      {
        videoId: "zATWKmpQGNI",
        title: "GMAT Geometry - Parth Joshi",
      },
      {
        videoId: "SlA8-gR_YM4",
        title: "GMAT Algebra - Shirish Verma",
      },
      {
        videoId: "YoOEYMSTdOo",
        title: "GMAT Geometry 1 - Shirish Verma",
      },
      {
        videoId: "570qYVl8RrA",
        title: "GMAT Geometry 2 - Shirish Verma",
      },
      {
        videoId: "htBQ-OlJmuQ",
        title: "GMAT Algebra 2 - Practice question - Shirish Verma",
      },
      {
        videoId: "7XmXogeRgXg",
        title: "GMAT Algebra 2 continued - Practice questions - Shirish Verma",
      },
      {
        videoId: "2mQM3og7WiA",
        title: "GMAT Probability - Shirish Verma",
      },
      {
        videoId: "K-aYWbBXdS0",
        title: "GMAT Probability 1 - Ankit",
      },
      {
        videoId: "oiORLypc5Ck",
        title: "GMAT Class Algebra - Ankit",
      },
      {
        videoId: "avmzO-IMp5Y",
        title: "GMAT Geometry 1 - Ankit",
      },
      {
        videoId: "opLTB5V9PkM",
        title: "GMAT Algebra 2 - Saahil Chouhan",
      },
      {
        videoId: "phE8Y5yeAU8",
        title: "GMAT Algebra 1 - Saahil Chouhan",
      },
      {
        videoId: "U5n4aM6NbUQ",
        title: "GMAT Probability 1 - Saahil Chouhan",
      },
      {
        videoId: "c_dCTvVFqUM",
        title: "GMAT Geometry 1 - Saahil Chouhan",
      },
      {
        videoId: "PK8lRS4EHhQ",
        title: "GMAT Geometry 2 - Saahil Chouhan",
      },
      {
        videoId: "zH6BXK7GIqk",
        title: "GMAT Probability - Saahil Chouhan",
      },
    ],
    [
      {
        videoId: "TLhf9eXoyRA",
        title: "GRE RC 2 - Hetal Salhotra",
      },
      {
        videoId: "lJCRdQPjN1E",
        title: "GRE RC 2 continued - Hetal Salhotra",
      },
      {
        videoId: "7GArcj0C3To",
        title: "GRE RC 3 - Hetal Salhotra",
      },
      {
        videoId: "w7FNoSZ4Fqw",
        title: "GRE RC 4 - Question solving - Hetal Salhotra",
      },
      {
        videoId: "PUtD0wmQ2Y4",
        title: "GMAT Sentence Correction 2 - Ankit",
      },
      {
        videoId: "k4-JM45voP0",
        title: "GMAT Sentence Correction 1 - Ankit",
      },
      {
        videoId: "hqQCVDe4fTM",
        title: "GRE RC 1 - Hetal Salhotra",
      },

      {
        videoId: "o4vTHyWjTxM",
        title: "BLOSSOM'S RC1 QUESTIONS SESSION-PARTH",
      },
      {
        videoId: "cu2vYsRUeec",
        title: "RC 2 and SC doubt clearing - Varun Salhotra",
      },
      {
        videoId: "BpVcrKv8LrA",
        title: "Adeft: Demo class - Verbal tutor Blossom",
      },
      {
        videoId: "xencbwPag4w",
        title: "GMAT BLOSSOM'S RC1 SESSION- ASITH",
      },
      {
        videoId: "rFk3Y_2wzxU",
        title: "GMAT CLASS WITH BLOSSOM RC1- Asith",
      },
      {
        videoId: "IIrPuWCKmEE",
        title: "GMAT CLASS WITH BLOSSOM CR2-  MANKIRAT",
      },
      {
        videoId: "Bmzb-53g9Gc",
        title: "GMAT CLASS WITH BLOSSOM CR1- MANKIRAT",
      },
      {
        videoId: "7GVggKKSW1Y",
        title: "GMAT CLASS WITH BLOSSOM RC2- MANKIRAT",
      },
      {
        videoId: "6zMeLJC5OCE",
        title: "19TH FEB GMAT CLASSES MANKIRAT WITH BLOSSOM- RC1",
      },
      {
        videoId: "teEMDkCr5Do",
        title: "GMAT Verbal accelerator session - Orientation for Varun",
      },
      {
        videoId: "HpjLxfD-4L8",
        title: "GMAT SC 1 - Varun Salhotra",
      },
      {
        videoId: "nI2HHuf2u6k",
        title: "GMAT SC 2 - Varun Salhotra",
      },
      {
        videoId: "vUIIOFwOKg8",
        title: "GMAT CR 1 - Varun Salhotra",
      },
      {
        videoId: "ARCm5S4kna8",
        title: "GMAT CR 2 - Varun Salhotra",
      },
      {
        videoId: "bdpJbYdB0ok",
        title: "GMAT SC | Doubt clearing session | Varun Salhotra",
      },
      {
        videoId: "mQUBlmFMy50",
        title: "GMAT RC 1 - Varun Salhotra",
      },
      {
        videoId: "vC65Ui_csIk",
        title: "GMAT CLASS WITH BLOSSOM RC2-ANUBHAV",
      },
      {
        videoId: "S6ukiDaN9Hk",
        title: "GMAT CLASS WITH BLOSSOM RC1-ANUBHAV",
      },
      {
        videoId: "VUuqahJq71U",
        title: "GMAT CR 1 continued and CR 2 - Anubhav Gupta",
      },
      {
        videoId: "osyg5CGsSgo",
        title: "GMAT CR 1 - Anubhav Gupta",
      },
      {
        videoId: "y1a8VR5gED4",
        title: "GMAT SC 2 - Anubhav Gupta",
      },
      {
        videoId: "zQyUz04Dkyc",
        title: "GMAT SC 1 continued - Anubhav Gupta",
      },
      {
        videoId: "u1rg8uID_sI",
        title: "GMAT SC 1 - Nithin Reddy",
      },
      {
        videoId: "5X6xINqaals",
        title: "GMAT SC 2 - Nithin Reddy",
      },
      {
        videoId: "j-b4T64yUtA",
        title: "GMAT CR 1 - Nithin Reddy",
      },
      {
        videoId: "SZV_PMnZ0xU",
        title: "GMAT CR 2 - Nithin Reddy",
      },
      {
        videoId: "59Rrid_v_OQ",
        title: "GMAT RC 2 - Nithin Reddy",
      },
      {
        videoId: "c6lcto8zHy8",
        title: "GMAT RC 1 CLASS- Parth Joshi",
      },
      {
        videoId: "7s7c3f6wSzE",
        title: "GMAT SC Doubt clearing 1 continued - Parth Joshi",
      },
      {
        videoId: "7AGdUmCxSSE",
        title: "GMAT SC Doubt clearing 1 - Parth Joshi",
      },
      {
        videoId: "iUoFs4PNxTg",
        title: "GMAT CR 2 - Parth Joshi",
      },
      {
        videoId: "Qh4c4L425MY",
        title: "GMAT CR 1 - Parth Joshi",
      },
      {
        videoId: "6BrXvcf0D6Y",
        title: "GMAT SC 1 continued 2 - Parth",
      },
      {
        videoId: "zeen0k4zYEM",
        title: "GMAT SC 1 continued - Parth Joshi",
      },
      {
        videoId: "jyOmvnYVfdM",
        title: "GMAT SC 1 - Parth Joshi",
      },
      {
        videoId: "thTN-RKLSGs",
        title: "GMAT SC 1 - Shirish Verma",
      },
      {
        videoId: "nS9gHGmxo-I",
        title: "GMAT SC 2 - Shirish Verma",
      },
      {
        videoId: "HPE-xNY5ilk",
        title: "GMAT RC 1 - Shirish Verma",
      },
      {
        videoId: "_z9aD-LIgAc",
        title: "GMAT CR 1 - Shirish Verma",
      },
      {
        videoId: "u0BVfsnq0RU",
        title: "GMAT RC 2 - Ankit",
      },
      {
        videoId: "Nql02WpcBV0",
        title: "GMAT RC 1 - Ankit",
      },
      {
        videoId: "hcx652XUiiE",
        title: "GMAT CR 2 - Ankit",
      },
      {
        videoId: "hu1cRvZXCJ8",
        title: "GMAT CR 1 continued - Ankit",
      },
      {
        videoId: "_X0gSBOKc5w",
        title: "GMAT - CR 1 - Ankit Rawat",
      },
      {
        videoId: "WiE6hqI1q9U",
        title: "GMAT SC Doubt Clearing - Ankit",
      },
      {
        videoId: "uIAIm9B4m08",
        title: "GMAT SC 1 - Saahil Chouhan",
      },
      {
        videoId: "q5GlINbi2q0",
        title: "GMAT SC 1 continued - Saahil Chouhan",
      },
      {
        videoId: "y0G6Wj2i2S4",
        title: "GMAT CR 1 - Saahil chouhan",
      },
      {
        videoId: "oCugv--Pfx4",
        title: "GMAT CR 2 - Saahil chouhan",
      },
      {
        videoId: "JQfwx4NyD0w",
        title: "GMAT RC 1 - Saahil chouhan",
      },
      {
        videoId: "PfcQ4e3MDI4",
        title: "GMAT RC 2 - Saahil Chouhan",
      },
    ],
    [
      {
        videoId: "RMTDXRMPj9c",
        title: "IR Session with Indradeep- Parth",
      },
      {
        videoId: "JOURL9Plbyw",
        title: "IR Questions with Answers",
      },
      {
        videoId: "9AxxRHnh8qA",
        title: "ASITH - CR1 Recording",
      },
      {
        videoId: "CYKOlkdVuBQ",
        title: "ASITH'S GMAT Classes- (KIRAN)",
      },
      {
        videoId: "KLq89ZD9dYc",
        title: "ASITH GMAT 4 APRIL (KIRAN)",
      },
      {
        videoId: "4zYk6rjTmTE",
        title: "GMAT trainer hiring: Kiran - Demo class",
      },
      {
        videoId: "MusIiKycHy0",
        title: "Demo class - Karthik | GMAT trainer hiring",
      },
      {
        videoId: "1EGdle0yiDs",
        title: "Demo class: Ghanshyam - GMAT hiring",
      },
      {
        videoId: "oMnoqu9Exkk",
        title: "GMAT IR - Nithin Reddy",
      },
      {
        videoId: "NT4K3PJHZLc",
        title: "GMAT Arithmetic 1 - Shirish Verma",
      },
      {
        videoId: "-zoFfY3Seaw",
        title: "GMAT Doubt clearing session 1 - Shirish",
      },
      {
        videoId: "OjrxvsbQ3Vo",
        title: "GMAT IR - Shirish Verma",
      },
    ],
    [
      {
        videoId: "Cmo2Y3h3BBI",
        title: "Admission consulting session 1 - Himanshu",
      },
      {
        videoId: "pypjh5d1zXg",
        title: "Profile evaluation session with Deepshika by Manas",
      },
      {
        videoId: "JcxgT7hGrps",
        title: "Profile evaluation session - Amarnath",
      },
      {
        videoId: "8_hDZgls6cI",
        title: "Profile evaluation session - Viraat",
      },
      {
        videoId: "lFdh0PWA7vM",
        title: "Profile evaluation session - Snehajeet",
      },
      {
        videoId: "PO0gLRFb-SY",
        title: "Profile evaluation meeting - Rishabh",
      },
      {
        videoId: "URBLkQAaJxs",
        title: "Admission consulting session 1 - Dilip Singh",
      },
      {
        videoId: "Ktl47SFVlrY",
        title: "Admission consulting session 1 - Kunal Doshi",
      },
      {
        videoId: "ZZRGggo5PDg",
        title: "Admission consulting session 2 - Kunal Doshi",
      },
      {
        videoId: "gHKu7fpsHGE",
        title: "Admission consulting session 3 - Kunal Doshi",
      },
      {
        videoId: "_nPJ9VCQHeM",
        title: "Admission consulting session 4 - Kunal Doshi",
      },
      {
        videoId: "SOLEFYoJWiA",
        title: "Admission consulting session 5 - Kunal",
      },
      {
        videoId: "AlOVVqOjOaw",
        title: "Admission Consulting Session 3 - Manikrat Khurrana",
      },
      {
        videoId: "AXMmqS1ZXYI",
        title: "Admission consulting session 2 - Mankirat Khuranna",
      },
      {
        videoId: "DP4n9JBRhpE",
        title: "Admission consulting session 1 - Mankirat",
      },
      {
        videoId: "ta3tV_aDb2E",
        title:
          "Building a winning application profile - Exclusive webinar for ROUND 1",
      },
      {
        videoId: "1GXu25rSt5o",
        title: "Admission consulting session - Mankirat",
      },
      {
        videoId: "zU0s51cfk3U",
        title: "Admission consulting session 1 - Varun Salhotra",
      },
      {
        videoId: "_X87CQxEDZQ",
        title: "Admission consulting session 2 - Varun Salhotra",
      },
      {
        videoId: "iOlTl24ZYSg",
        title: "Admission consulting session 3 - Varun Salhotra",
      },
      {
        videoId: "PEw3nCNy7mY",
        title: "Admission consulting session 1 - Apurva",
      },
      {
        videoId: "gbKb5tLf5FU",
        title: "Admission consulting session 2 - Apurva",
      },
      {
        videoId: "JFQIERvVW1Q",
        title: "Admission consulting session 1 - Ameya",
      },
      {
        videoId: "8NDqkbdzA4s",
        title: "Admission consulting session 2 - Ameya",
      },
      {
        videoId: "yF8Ftb9or-E",
        title: "Admission consulting session 3 - Ameya",
      },
      {
        videoId: "YmiCf0UfxE0",
        title: "Admission consulting session 4 - Ameya",
      },
      {
        videoId: "iswbXS8yzbY",
        title: "Admission consulting session 5 - Ameya",
      },
      {
        videoId: "0A1BbItD54c",
        title:
          "GMAT and Admission consulting | Orientation session | Nithin Reddy",
      },
      {
        videoId: "_jzFa3Y3mTI",
        title: "Admission consulting session - Subin Ram Mohan",
      },
      {
        videoId: "RbNT0MsQHJc",
        title: "Admission consulting session 1 - Varun Sharma",
      },
      {
        videoId: "pqRAC8bm3ng",
        title: "Profile evaluation meeting for Varun Sharma",
      },
      {
        videoId: "2QaoihBPgjI",
        title: "Admission consulting session 2 - Varun sharma",
      },
      {
        videoId: "k5sRWWAN_-A",
        title: "Admission consulting session 1 - Suhail",
      },
      {
        videoId: "-0EVylHuMJg",
        title: "Profile evaluation meeting for Suhail - with Indradeep",
      },
      {
        videoId: "wvKowcmUT_0",
        title: "Admission consulting session 2 - Suhail",
      },
      {
        videoId: "zEMMIg_LoGA",
        title: "Admission consulting session 3 - Suhail",
      },
      {
        videoId: "UWGb6949Kjs",
        title: "Admission consulting session 4 - Suhail",
      },
      {
        videoId: "ffLyGEia7JE",
        title: "Admission consulting session 5 - Suhail",
      },
      {
        videoId: "uxD9TSm5JtI",
        title: "Admission consulting session 1 - Timila Garg",
      },
      {
        videoId: "04cNfsnWl3k",
        title: "Admission consulting session 2 - Timila",
      },
      {
        videoId: "2v2viuVAojg",
        title: "Admission consulting session 3 - Timila",
      },
      {
        videoId: "ZPsqNSwkteI",
        title: "Admission consulting session 1 - Kapil",
      },
      {
        videoId: "dSEHswMUtQg",
        title: "Admission consulting session 2 - Kapil",
      },
      {
        videoId: "DFq6UrVXpfo",
        title: "Profile evaluation meeting for Kapil",
      },
      {
        videoId: "Jmgi-r6gN7M",
        title: "Admission consulting session 1 - Furqan",
      },
      {
        videoId: "M9a-HC8PI0k",
        title: "Admission consulting session 2 - Furqan",
      },
      {
        videoId: "UjzhfgiO7n4",
        title: "Admission consulting session 3 - Furqan",
      },
      {
        videoId: "Icv39KrsHVg",
        title: "Admission consulting session 1 - Lakshya Chopra",
      },
      {
        videoId: "peWutBioXtU",
        title: "Admission consulting session 2 - Lakshya Chopra",
      },
      {
        videoId: "ueFUApKQQxo",
        title: "Admission consulting session 3 - Lakshya Chopra",
      },
      {
        videoId: "4qdZX7Yj0Q0",
        title: "Admission consulting session 4 - Lakshya Chopra",
      },
      {
        videoId: "6egFymS0ueY",
        title: "Admission consulting session 5 - Lakshya Chopra",
      },
      {
        videoId: "p38JhzBfHbg",
        title: "Admission consulting session 6 - Lakshya chopra",
      },
      {
        videoId: "qKuQy01QkuE",
        title: "Admission consulting session 7 - Lakshya chopra",
      },
      {
        videoId: "oCDKkjUDNhA",
        title: "Admission consulting session 1 - Kaustav Karmakar",
      },
      {
        videoId: "oS7xpUKldEQ",
        title: "Admission consulting session 2 - Kaustav Karmakar",
      },
      {
        videoId: "zxgLk8rxQlE",
        title: "Admission consulting session 3 - Kaustav",
      },
      {
        videoId: "h0xOSs_ZsHA",
        title: "Admission consulting session 4 - Kaustav",
      },
      {
        videoId: "3p_1zm17rUI",
        title: "Admission consulting session 1 - Tirth Ganatra",
      },
      {
        videoId: "mOh0G8Ds5Aw",
        title: "Admission consulting session 2 - Tirth Ganatra",
      },
      {
        videoId: "gYN9QoiHpdU",
        title: "Admission consulting session 3 - Tirth Ganatra",
      },
      {
        videoId: "c18gVerEA8w",
        title: "Admission consulting session 4 - Tirth Ganatra",
      },
      {
        videoId: "430eqzplX3U",
        title: "Admission consulting session - 5 (Incomplete recording)",
      },
      {
        videoId: "vBmwkrnk754",
        title: "Admission consulting session 6 - Tirth Ganatra",
      },
      {
        videoId: "vRyVEdfcrFQ",
        title:
          "Admission consulting session 7 (Mock Interview) - Tirth Ganatra",
      },
      {
        videoId: "E3iFCbtZGKo",
        title: "Admission consulting session 8 - Tirth Ganatra",
      },
      {
        videoId: "UHto9yOmawQ",
        title: "Admission consulting session 5 - Madhav Srinivas",
      },
      {
        videoId: "R7XssMikf2E",
        title: "Admission consulting session 2 - Madhav Srinivas",
      },
      {
        videoId: "YhBXUtBY6B4",
        title: "Admission consulting session 1 - Madhav Srinivas",
      },
      {
        videoId: "a5chauGvyGY",
        title: "Admission Consulting session 3 - Madhav",
      },
      {
        videoId: "JQo7FY-6mTA",
        title: "Admission consulting session 6 - Madhav Srinivas",
      },
      {
        videoId: "fyB8Xk9-8qo",
        title: "Admission consulting session 1 - Prateek Bindal",
      },
      {
        videoId: "KmeQrh0z7eU",
        title: "Admission consulting session 2 - Prateek Bindal",
      },
      {
        videoId: "soWtb88Ekvs",
        title: "Profile evaluation meeting with Prateek bindal",
      },
      {
        videoId: "_HTm1g_GH7U",
        title: "Admission consulting session 3 - Prateek Bindal",
      },
      {
        videoId: "-DgAhuFui1k",
        title: "Admission consulting session 4 - Prateek Bindal",
      },
      {
        videoId: "hKOoGOqT_WQ",
        title: "Admission consulting session 5 - Prateek Bindal",
      },
      {
        videoId: "5BU11ttOUFk",
        title: "Profile evaluation meeting with Mani Pranay",
      },
      {
        videoId: "soWtb88Ekvs",
        title: "Profile evaluation meeting with Prateek bindal",
      },
      {
        videoId: "Wo8w7HcWioY",
        title: "Profile evaluation meeting with Shirish",
      },
      {
        videoId: "6mDcRuhDEAI",
        title: "Profile evaluation meeting with Vamsi",
      },
      {
        videoId: "kNfTMWyMF-U",
        title: "Profile evaluation meeting with Prateek",
      },
      {
        videoId: "9h0nzqFgXWY",
        title: "Profile evaluation meeting with Radhika",
      },
      {
        videoId: "d1MNLifYP40",
        title: "Profile evaluation meeting with Dona Jain",
      },
      {
        videoId: "zDpww2ORquE",
        title: "Profile evaluation meeting with Shrijan Sharma",
      },
      {
        videoId: "WcRSbCkwGRo",
        title: "Profile evaluation for Shivam Arora",
      },
      {
        videoId: "McrM5ee0pDo",
        title: "Profile evaluation for Furquan",
      },
      {
        videoId: "zAZqfw9qYUY",
        title: "Profile evaluation session - Vivek Gupta",
      },
      {
        videoId: "MeJafkmQjZs",
        title: "Profile evaluation for Puneeth",
      },
      {
        videoId: "DFq6UrVXpfo",
        title: "Profile evaluation meeting for Kapil",
      },
      {
        videoId: "UUgo-FThxGA",
        title: "Profile evaluation meeting with Prateek Sri",
      },
      {
        videoId: "hM2Lq8cqK-8",
        title: "Profile evaluation meeting with Bhavya",
      },
      {
        videoId: "F9j8aWl4pgE",
        title: "Profile evaluation meeting with Shubam",
      },
      {
        videoId: "Zx87NUpAsPE",
        title: "Profile evaluation meeting with Vaibhav",
      },
      {
        videoId: "RCn4xz03hJE",
        title: "Profile evaluation meeting for Timila",
      },
      {
        videoId: "llmWQ9PStIM",
        title: "Profile evaluation meeting for Mithilesh",
      },
      {
        videoId: "nLhy6TGf17Q",
        title: "Profile evaluation meeting for Sukhbir",
      },
      {
        videoId: "uoQ7Uyjyib8",
        title: "Profile evaluation meeting for Akarsh",
      },
      {
        videoId: "rt0ETtQ3xOA",
        title: "Profile evaluation session for Abishek",
      },
      {
        videoId: "BsAphsDAKBc",
        title: "Profile evaluation session for Pranav",
      },
      {
        videoId: "-0EVylHuMJg",
        title: "Profile evaluation meeting for Suhail - with Indradeep",
      },
      {
        videoId: "pqRAC8bm3ng",
        title: "Profile evaluation meeting for Varun Sharma",
      },
      {
        videoId: "XaaOMIjui9s",
        title: "Profile evaluation meeting with Lisa",
      },
      {
        videoId: "1q0iYtwNMSg",
        title: "Profile evaluation meeting with Konan (Kaunin Ali)",
      },
      {
        videoId: "5K8q08K86lI",
        title: "Profile evaluation session - Mohammed Shuib",
      },
      {
        videoId: "anQIWn0UMEw",
        title: "Profile evaluation session for Ankit Goel",
      },
      {
        videoId: "RpAu5-dbqoU",
        title: "Profile evaluation meeting -Shreya Arora",
      },
      {
        videoId: "NQKMq2MNMv8",
        title: "Profile evaluation meeting - Ashritha",
      },
      {
        videoId: "LABFE_kxAnQ",
        title: "Profile evaluation meeting - Vishal Kore",
      },
      {
        videoId: "j-45PLjSpYg",
        title: "Profile evaluation meeting with Aditya",
      },
      {
        videoId: "KNcjFs8-jEs",
        title: "Profile evaluation session - Mankirat",
      },
      {
        videoId: "UXIoB4hyuAI",
        title: "Profile evaluation session - Satyaket",
      },
      {
        videoId: "a6AwKcUP_t0",
        title: "Admission consulting session 2 - Nitesh",
      },
      {
        videoId: "JfOBYLVuth8",
        title: "Admission consulting session 3 - Nitesh",
      },
      {
        videoId: "ibFjlF9NPKM",
        title: "Admission consulting session 1 - Nitesh",
      },
      {
        videoId: "X8pGUVOIPNs",
        title: "Admission Consulting session 8 - Ankit",
      },
      {
        videoId: "yUPDlcuCWBw",
        title: "Admission consulting session 7 - Ankit",
      },
      {
        videoId: "cAhzjOL31Qk",
        title:
          "GMAT doubt clearing and Admission consulting session 7 - Ankit Rawat",
      },
      {
        videoId: "polNc-Xjd9k",
        title: "Admission consulting session 6 - Ankit",
      },
      {
        videoId: "8ZqZSMKMkiU",
        title: "Admission Consulting session 4 - Ankit",
      },
      {
        videoId: "4PDq_iJrVjE",
        title: "Admission Consulting session 2 - Ankit",
      },
      {
        videoId: "7iTDB6G-vyE",
        title: "GMAT IR and Admission consulting session 9 - Ankit Rawat",
      },
      {
        videoId: "dHyR0jdjUUk",
        title: "Admission consulting session 10 - Ankit Rawat",
      },
      {
        videoId: "ebJmsqUs9fU",
        title: "Admission consulting session 11 - Ankit Rawat",
      },
      {
        videoId: "uneMz9QjNSE",
        title: "Admission consulting session 12 - Ankit Rawat",
      },
      {
        videoId: "ocg08-s5WkA",
        title: "Admission consulting session 13 - Ankit Rawat",
      },
      {
        videoId: "qbUjq5nvmY8",
        title: "Admission consulting session 14 - Ankit Rawat",
      },
      {
        videoId: "KOUVEn9eyaA",
        title: "Admission consulting session 15 - Ankit",
      },
      {
        videoId: "ta3tV_aDb2E",
        title:
          "Building a winning application profile - Exclusive webinar for ROUND 1",
      },
      {
        videoId: "q5FYAAqr__8",
        title: "Admission consulting session 2 - Shaista",
      },
      {
        videoId: "maiOodjNYUg",
        title: "Admission consulting session 1 - Shaista Aafreen",
      },
      {
        videoId: "3Y3dX_AGRo0",
        title: "Admission consulting session 3 - Shaista",
      },
      {
        videoId: "pgklY0MSosg",
        title: "Admission Consulting session 4 - Shaista",
      },
      {
        videoId: "7lwdrMFGork",
        title: "Admission consulting session 5 - Shaista",
      },
      {
        videoId: "0Zwu1Hl0BhI",
        title: "Admission consulting session 6 - Shaista Aafreen",
      },
    ],
    [
      {
        videoId: "0poqJSWiZc4",
        title: "Career options after MBA",
      },
      {
        videoId: "JCigrAao3dY",
        title: "Admission Counselling- Asith",
      },
      {
        videoId: "6W0QaLN4P9E",
        title: "Admission Counselling- Swarnava",
      },
      {
        videoId: "apo4r8QNFTw",
        title: "Your unique life story for MBA admissions - Indradeep Mazumdar",
      },
      {
        videoId: "mJHrneLFWYo",
        title: "Key things admissions committees look for - Indradeep Mazumdar",
      },
      {
        videoId: "ZB3bVQ2_VcQ",
        title:
          "Answering Wharton essays - Know about Top MBA essays. An Informative series",
      },
      {
        videoId: "FAGslxEjWtg",
        title: "5 Most common MBA Essays - Live webinar by Indradeep Mazumdar",
      },
      {
        videoId: "6IHjmV-jJfs",
        title: "Debunking the myths of MBA Admission - Live webinar",
      },
      {
        videoId: "Qgf3dDE8a5c",
        title:
          "Adeft Webinar: MBA Admissions - Last second tricks and checklist before you hit submit",
      },
      {
        videoId: "RYcRdpnbz_U",
        title:
          "Testimonial | Tirth Ganatra | ESCP university admission holder along with Scholarship",
      },
      {
        videoId: "I2NPLZHHdw4",
        title: "Admission Strategy session 1 - Ankit",
      },
      {
        videoId: "6IHjmV-jJfs",
        title: "Debunking the myths of MBA Admission - Live webinar",
      },
      {
        videoId: "FAGslxEjWtg",
        title: "5 Most common MBA Essays - Live webinar by Indradeep Mazumdar",
      },
      {
        videoId: "Qgf3dDE8a5c",
        title:
          "Adeft Webinar: MBA Admissions - Last second tricks and checklist before you hit submit",
      },
      {
        videoId: "ZB3bVQ2_VcQ",
        title:
          "Answering Wharton essays - Know about Top MBA essays. An Informative series",
      },
    ],
    [
      {
        videoId: "MqktAocqJ-Y",
        title: "Your career goals",
      },
      {
        videoId: "q42UOQwxzzk",
        title:
          "Testimonial | Manas Srivastava | Cornell Graduate MBA Class of 2020-21",
      },
      {
        videoId: "Ya8nc49MroE",
        title: "Ankit's video testimonial for GMAT Coaching",
      },
      {
        videoId: "Ya8nc49MroE",
        title: "Ankit's video testimonial for GMAT Coaching",
      },
    ],
    [
      {
        videoId: "x10C_QLJ1Bw",
        title: "Asith GMAT Classes- Indradeep",
      },
      {
        videoId: "EUTdtXxkQvE",
        title: "ASITH'S GMAT Classes- Ghanshyam",
      },
      {
        videoId: "l5Ax4sttENg",
        title: "ASITH'S GMAT CLASS- Ghanshyam",
      },

      {
        videoId: "ylAtg1MbZQA",
        title: "21/03/2022 - ASITH GMAT 1",
      },
      {
        videoId: "Hw9DCP22ojM",
        title: "21/03/2022 - ASITH GMAT -  2",
      },
      {
        videoId: "cpkoTC9k6n0",
        title: "ASITH GMAT 27 MAR",
      },

      {
        videoId: "kCbpA1Mkkks",
        title: "KUNAL'S MOCK INTERVIEW WITH INDRADEEP",
      },
      {
        videoId: "C_QkpX_qL6c",
        title: "14/03/2022 Q/A DISCUSSION - MANIKRAT",
      },

      {
        videoId: "adBeErFO43I",
        title: "Post MBA Short Term Goal",
      },

      {
        videoId: "4N5VEVj7Gr0",
        title: "Study and Work Abroad - Indradeep Mazumdar",
      },
      {
        videoId: "kiOiuanML9U",
        title:
          "Webinar: How to make your applications stand out - Exclusively for round 2 applicants",
      },
      {
        videoId: "wwynnkOVods",
        title: "GMAT Doubt clearing session - Varun Salhotra",
      },
      {
        videoId: "ASIk5g9wx2U",
        title: "GMAT Doubt clearing session 2 - Varun Salhotra",
      },

      {
        videoId: "QZEQDQB6MBs",
        title: "ANUBHAV 27 MAR GMAT",
      },

      {
        videoId: "06pmpd5PkPc",
        title: "Anubhav's GMAT classes with Indradeep- Data Sufficiency",
      },
      {
        videoId: "JsswsD7EJx4",
        title: "Anubhav's GMAT CLASS- Indradeep",
      },
      {
        videoId: "D_YnCxgCng8",
        title: "GMT20220205 094931 Recording 1686x768",
      },
      {
        videoId: "diNPmyDQN-w",
        title: "GMAT Doubt clearing session 1 - Anubhav",
      },

      {
        videoId: "6IIhy9hMPbI",
        title: "KSS 4 | Research activity | Nava and syeda",
      },
      {
        videoId: "57eDqTTEvhk",
        title: "KSS 1 - L2 calls - Arghya and Syeda",
      },

      {
        videoId: "Xi5WjIpyqjk",
        title: "GMAT CLASS BLOSSOM'S AWA SESSION- NITHIN",
      },
      {
        videoId: "yRveYnorgVI",
        title: "PARTH GMAT 21 APR (BLOSSOM)",
      },
      {
        videoId: "IJDOL4YGYLw",
        title: "PARTH GMAT 16 APR",
      },

      {
        videoId: "seu9HS43_-8",
        title: "Pre GMAT session - Parth Joshi",
      },
      {
        videoId: "fS8fI4nNp-o",
        title: "GMAT Class - Orientation - Parth Joshi",
      },
      {
        videoId: "mnl7-BY17NQ",
        title: "Orientation for Shirish Verma",
      },
      {
        videoId: "pwrDR4PV-24",
        title: "Orientation for Hetal",
      },

      {
        videoId: "KMmbuTG5JBE",
        title: "GRE Mock review - Hetal Salhotra",
      },
      {
        videoId: "VxaHm8Nmr6k",
        title: "GRE Doubt clearing session 1 - Hetal Salhotra",
      },
      {
        videoId: "IM-5IUd4DFE",
        title: "HETAL (BLOSSOM) 27 MAR",
      },
      {
        videoId: "_n-YcKzWwwo",
        title: "HETAL GMAT 03 APRIL (BLOSSOM)",
      },
      {
        videoId: "sx7ExhThiBg",
        title: "HETAL GMAT 06 APR (BLOSSOM)",
      },
      {
        videoId: "cSvgXVQML4Y",
        title: "HETAL GMAT 16 APR",
      },
      {
        videoId: "E8e1ZFZOqPM",
        title: "General meeting with Madhav srinivas",
      },
      {
        videoId: "C80nC9ENi7M",
        title: "Meeting with Shraddha's dad",
      },
      {
        videoId: "2kOhT7rwgZA",
        title: "GMAT Doubt Clearing session 3 - Ankit",
      },
      {
        videoId: "f3kHekELdnw",
        title: "Doubt Clearing session 2 - Ankit",
      },

      {
        videoId: "7sEmPEFI0LU",
        title: "GMAT Doubt clearing session 4 - Ankit Rawat",
      },
      {
        videoId: "3HzA7xinDDk",
        title:
          "Unique strategies to attempt Verbal questions - Live Interactive webinar",
      },
      {
        videoId: "xO2mjfm52Ss",
        title:
          "ISB webinar - Getting the best out of an MBA ft. ISB Alum Anusha Rajagopalan and Tarun Mansukhani",
      },
      {
        videoId: "8xM_ATdCwho",
        title:
          "Webinar: Live question solving of CR and Unique strategies to get adapted by Indradeep Mazumdar",
      },
      {
        videoId: "kiOiuanML9U",
        title:
          "Webinar: How to make your applications stand out - Exclusively for round 2 applicants",
      },
      {
        videoId: "RSCWJGG46Qs",
        title: "GMAT Doubt clearing session - Shaista",
      },
      {
        videoId: "AalemGQbCIE",
        title: "Orientation for Shaista Aafreen",
      },
      {
        videoId: "r8M3MIjvEVo",
        title: "Orientation for Saahil",
      },

      {
        videoId: "8miknCFPWEY",
        title: "GMAT Doubt clearing session 1 - Saahil Chouhan",
      },
      {
        videoId: "LaSSQX3vw5w",
        title: "GMAT Doubt clearing session 2 - Saahil chouhan",
      },
      {
        videoId: "X4oUQGS88Pw",
        title: "GMAT Doubt clearing session 3 - Saahil Chouhan",
      },
      {
        videoId: "y_5MYyU5dJE",
        title: "GMAT Doubt clearing session 4 - Saahil chouhan",
      },
      {
        videoId: "Qucti2yPKoo",
        title: "GMAT Doubt clearing session 5 - Saahil chouhan",
      },
      {
        videoId: "wFZlEP9s24w",
        title: "GMAT Doubt clearing session 7 - Saahil Chouhan",
      },
      {
        videoId: "lEKW4Jt0lp8",
        title: "GMAT Doubt clearing session 6 - Saahil Chouhan",
      },
      {
        videoId: "tjo5jGqTqM4",
        title: "GMAT Doubt clearing session 8 - Saahil Chouhan",
      },
      {
        videoId: "ClhHU77YJB8",
        title: "GMAT Doubt clearing session 9 - Saahil chouhan",
      },
    ],
    // Add more categories
  ];

  return (
    <div>
      <NavBar />
      <div style={{ marginTop: "100px" }}>
        <div className="section-title my-3">
          <h2>Adeft Consulting</h2>
        </div>
        {categories.map((videos, index) => (
          <div key={index} className="mb-3">
            <div className="ml-3  text-left d-flex justify-content-between align-items-center mb-3">
              <h4 className="text-primary" style={{ fontWeight: "bold" }}>
                {categoryNames[index]}
              </h4>
              <Link
                style={{ marginRight: "20px" }}
                to={`/${categoryNames[index]
                  .replace(/\s+/g, "-")
                  .toLowerCase()}`}
              >
                <span className=" d-flex justify-content-between align-items-center">
                  {" "}
                  See more {""} <RightOutlined />
                </span>
              </Link>
            </div>
            <VideoCarousel videos={videos} />
          </div>
        ))}
      </div>
    </div>
  );
};

export default VideosPage;
